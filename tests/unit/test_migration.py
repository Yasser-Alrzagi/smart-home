...
MetaData, which is then compared field by field against ``app.models.Base.metadata``.
...
def test_upgrade_creates_all_22_tables(upgraded):
    ...
    assert set(upgraded.metadata.tables) == set(Base.metadata.tables)


@pytest.mark.parametrize("table_name", sorted(Base.metadata.tables))
def test_table_matches_the_model(upgraded, table_name):
    assert _fingerprint(upgraded.metadata.tables[table_name]) == _fingerprint(
...
def test_downgrade_drops_every_table_once(downgraded):
    ...
    assert set(downgraded.dropped_tables) == set(Base.metadata.tables)
...
def test_downgrade_never_drops_a_table_that_is_still_referenced(downgraded):
    ...
    remaining = set(Base.metadata.tables)
    for name in downgraded.dropped_tables:
        remaining.discard(name)
        blockers = [
            other
            for other in remaining
            for fk in Base.metadata.tables[other].foreign_keys
...
def test_every_index_in_the_models_is_created_and_dropped(upgraded, downgraded):
    ...
    assert {name for name, _ in downgraded.dropped_indexes} == model_indexes


def test_no_invented_schema_elements(upgraded):
    """Nothing in the migration may exist that the approved models do not declare."""
    extra_tables = set(upgraded.metadata.tables) - set(Base.metadata.tables)
