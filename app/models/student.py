...
class Student(Base):
    ...
    __table_args__ = (
        CheckConstraint("profile_version >= 1", name="ck_students_profile_version"),
    )
    ...
    academic_status = Column(
        SAEnum(AcademicStatus), nullable=False, default=AcademicStatus.continuing
    )
    housing_status = Column(
        SAEnum(HousingStatus), nullable=False, default=HousingStatus.applicant
    )
    ...
    status_history = relationship(
        ...
        order_by="StudentStatusHistory.start_date",
    )
    room_assignments = relationship(
        "RoomAssignment", back_populates="student", cascade="all, delete-orphan"
    )
    complaints = relationship(
        "Complaint", back_populates="student", cascade="all, delete-orphan"
    )
    maintenance_requests = relationship(
        "MaintenanceRequest", back_populates="student", cascade="all, delete-orphan"
    )
    permission_requests = relationship(
        "PermissionRequest", back_populates="student", cascade="all, delete-orphan"
    )
    absences = relationship(
        "StudentAbsence", back_populates="student", cascade="all, delete-orphan"
    )
    emergency_reports = relationship(
        "EmergencyReport", back_populates="student", cascade="all, delete-orphan"
    )
    service_registrations = relationship(
        "ServiceRegistration", back_populates="student", cascade="all, delete-orphan"
    )
    disciplinary_cases = relationship(
        "DisciplinaryCase", back_populates="student", cascade="all, delete-orphan"
    )
    cleaning_assignments = relationship(
