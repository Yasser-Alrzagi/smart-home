...
(() => {
  ...
  const serviceTypes = {'Activity':'أنشطة','Internet':'إنترنت','Food':'تغذية','Sports':'رياضة'};
  const periodStatuses = {'Upcoming':'قادم','Open':'مفتوح للتسجيل','Closed':'مغلق','Completed':'مكتمل'};
  const registrationStatuses = {'Registered':'مسجل','Cancelled':'ملغي','Completed':'مكتمل'};
  const complaintStatuses = {'Open':'مفتوح','Under Review':'قيد المراجعة','Resolved':'تمت المعالجة','Closed':'مغلق'};
  const maintenanceStatuses = {'Pending':'بانتظار الإسناد','Assigned':'مُسند','In Progress':'جاري التنفيذ','Resolved':'تمت المعالجة','Closed':'مغلق'};
  ...
  function dataTable(headers, rows){return h('div',{class:'table-scroll'},h('table',{},h('thead',{},h('tr',{},headers.map(x=>h('th',{},x)))),h('tbody',{},rows.map(cells=>h('tr',{},cells.map(c=>h('td',{},c)))))));}
  const d5badge=(status,ok,wait,ko)=>h('span',{class:'badge '+(status===ok?'accepted':status===ko?'rejected':'waiting')},tr(status));
  async function servicesPage(){const epoch=viewEpoch;if(state.page!=='services'||!state.me)return;
    if(['Activity Officer','Food Officer','Sports Officer'].includes(state.me.role))return officerServices(epoch);
    const periods=await api('/service-periods');const mine=await api('/service-registrations/me');const mineBy=Object.fromEntries(mine.map(r=>[r.period_id,r]));
    const open=periods.filter(p=>['Upcoming','Open'].includes(p.status));
    const cards=open.length?h('div',{class:'cards'},...open.slice(0,9).map(p=>h('div',{class:'stat'},h('span',{class:'stat-label'},tr(p.service_type)+' · '+p.service_name),h('div',{class:'stat-value'},p.seats+' / '+(p.capacity??'∞')),h('small',{},tr(p.status)+' · من '+p.start_date+' إلى '+p.end_date),h('small',{},mineBy[p.period_id]?'مسجل: '+tr(mineBy[p.period_id].status):'غير مسجل')))):h('p',{class:'muted'},'لا توجد فترات مفتوحة الآن.');
    const mineTable=mine.length?dataTable(['الخدمة','الفترة','سجّلت في','الحالة',''],mine.map(r=>[tr(r.service_type)+' · '+r.service_name,r.start_date+' → '+r.end_date,date(r.registered_at),d5badge(r.status,'Registered',null,'Cancelled'),r.status==='Registered'?button('إلغاء التسجيل',async()=>{if(!window.confirm('إلغاء تسجيلك في هذه الفترة؟'))return;try{await api('/service-registrations/'+r.registration_id+'/cancel',{method:'POST'});notify('أُلغي التسجيل.');await servicesPage();}catch(err){notify(err.message,true);}},'danger'):''])):empty('لا تسجيلات لك','سجّل في فترة مفتوحة من البطاقات أعلاه.');
    const slot=open.filter(p=>p.status==='Open'&&!mineBy[p.period_id]);
    const registerForm=h('form',{class:'inline-form'},h('label',{},'الفترة',h('select',{name:'period_id'},slot.length?slot.map(p=>h('option',{value:p.period_id},tr(p.service_type)+' · '+p.service_name+' ('+p.start_date+' → '+p.end_date+')')):h('option',{value:''},'لا فترات مفتوحة'))),h('button',{type:'submit',class:'primary'},'سجّل الآن'));
    registerForm.addEventListener('submit',async e=>{e.preventDefault();const b=registerForm.querySelector('button');b.disabled=true;try{await api('/service-registrations',{method:'POST',body:{period_id:registerForm.querySelector('select').value}});notify('تم التسجيل بنجاح.');await servicesPage();}catch(err){notify(err.message,true);}finally{b.disabled=false;}});
    renderPage('services',epoch,heading('الخدمات والتسجيلات','سجّل في الأنشطة والإنترنت والتغذية والرياضة وفق الفترات المفتوحة.'),panel('الفترات',cards),panel('تسجيل سريع',registerForm),panel('تسجيلاتي',mineTable));
  }
  async function officerServices(epoch){const services=await api('/services');const periods=await api('/service-periods');
    const createService=h('form',{class:'inline-form'},h('label',{},'النوع',h('select',{name:'service_type'},Object.entries(serviceTypes).map(([v,l])=>h('option',{value:v},l)))),field('الاسم','name','','text',{minlength:2,maxlength:200}),h('button',{type:'submit',class:'primary'},'إضافة خدمة'));
    createService.addEventListener('submit',async e=>{e.preventDefault();const b=createService.querySelector('button');b.disabled=true;try{const fd=Object.fromEntries(new FormData(createService));await api('/services',{method:'POST',body:{service_type:fd.service_type,name:fd.name,is_active:true}});notify('أُضيفت الخدمة.');await officerServices(epoch);}catch(err){notify(err.message,true);}finally{b.disabled=false;}});
    const serviceSel=h('select',{name:'service_id'},services.map(x=>h('option',{value:x.service_id},tr(x.service_type)+' · '+x.name)));
    const createPeriod=h('form',{class:'inline-form'},serviceSel,field('من','start_date','','date'),field('إلى','end_date','','date'),field('السعة','capacity','','number',{min:1,max:1000}),h('button',{type:'submit',class:'primary'},'إضافة فترة'));
    createPeriod.addEventListener('submit',async e=>{e.preventDefault();const b=createPeriod.querySelector('button');b.disabled=true;try{const fd=Object.fromEntries(new FormData(createPeriod));await api('/service-periods',{method:'POST',body:{service_id:fd.service_id,start_date:fd.start_date,end_date:fd.end_date,capacity:Number(fd.capacity||0)||null}});notify('أُضيفت الفترة.');await officerServices(epoch);}catch(err){notify(err.message,true);}finally{b.disabled=false;}});
    async function setPeriod(per,status){try{await api('/service-periods/'+per.period_id+'/status',{method:'POST',body:{status}});notify('حُدّثت حالة الفترة.');await officerServices(epoch);}catch(err){notify(err.message,true);}}
    async function listRegistrations(per){const page=await api('/service-periods/'+per.period_id+'/registrations');dialog('تسجيلات — '+per.service_name+' ('+per.start_date+' → '+per.end_date+')',page.items.length?dataTable(['الطالب','الجامعة','سجّل في','الحالة'],page.items.map(r=>[r.student_name,r.university,date(r.registered_at),tr(r.status)])):h('p',{class:'muted'},'لا تسجيلات بعد.'),h('div',{class:'actions'},button('إغلاق',()=>$('detailDialog').close())));}
    function periodActions(per){const acts=[];if(per.status==='Upcoming')acts.push(button('فتح التسجيل',()=>setPeriod(per,'Open'),'primary'));if(per.status==='Open')acts.push(button('إغلاق',()=>setPeriod(per,'Closed')));if(per.status==='Closed')acts.push(button('إعادة فتح',()=>setPeriod(per,'Open'),'primary'),button('إكمال',()=>setPeriod(per,'Completed')));acts.push(button('التسجيلات',()=>listRegistrations(per)));return h('span',{class:'file-actions'},acts);}
    const periodTable=periods.length?dataTable(['الخدمة','الفترة','السعة','الحالة',''],periods.map(p=>[tr(p.service_type)+' · '+p.service_name,p.start_date+' → '+p.end_date,p.seats+' / '+(p.capacity??'∞'),d5badge(p.status,'Open','Upcoming','Completed'),periodActions(p)])):empty('لا فترات بعد','أضف فترة للخدمة التي تديرها.');
    renderPage('services',epoch,heading('إدارة الخدمات','أنشئ الخدمات والفترات، وافتح التسجيل، وتابع المقاعد.'),panel('إضافة خدمة',createService),panel('إضافة فترة',createPeriod),panel('الفترات',periodTable));
  }
  async function supportPage(){const epoch=viewEpoch;if(state.page!=='support'||!state.me)return;
    if(state.me.role==='Student')return studentSupport(epoch);
    if(state.me.role==='Housing Administration')return complaintsAdmin(epoch);
    if(state.me.role==='Maintenance Officer')return maintenanceAdmin(epoch);
    renderPage('support',epoch,heading('الدعم والبلاغات','لا توجد صلاحيات لهذا الدور.'),h('p',{class:'muted'},'لا يمكنك الوصول إلى هذه الصفحة.'));}
  async function studentSupport(epoch){const complaints=await api('/complaints/my');const maintenance=await api('/maintenance/my');
    const complaintForm=h('form',{class:'inline-form'},field('التصنيف','category','','text',{minlength:2,maxlength:100}),field('الوصف','description','','text',{minlength:3,maxlength:2000}),h('button',{type:'submit',class:'primary'},'تقديم شكوى'));
    complaintForm.addEventListener('submit',async e=>{e.preventDefault();const b=complaintForm.querySelector('button');b.disabled=true;try{await api('/complaints',{method:'POST',body:Object.fromEntries(new FormData(complaintForm))});notify('وصلت شكواك.');await studentSupport(epoch);}catch(err){notify(err.message,true);}finally{b.disabled=false;}});
    const maintenanceForm=h('form',{class:'inline-form'},field('نوع العطل','problem_type','','text',{minlength:2,maxlength:100}),field('الوصف','description','','text',{minlength:3,maxlength:2000}),h('button',{type:'submit',class:'primary'},'تقديم طلب صيانة'));
    maintenanceForm.addEventListener('submit',async e=>{e.preventDefault();const b=maintenanceForm.querySelector('button');b.disabled=true;try{await api('/maintenance',{method:'POST',body:Object.fromEntries(new FormData(maintenanceForm))});notify('وصل طلب الصيانة.');await studentSupport(epoch);}catch(err){notify(err.message,true);}finally{b.disabled=false;}});
    const complaintTable=complaints.items.length?dataTable(['التصنيف','الوصف','الحالة','أُنشئ في'],complaints.items.map(c=>[c.category,c.description,d5badge(c.status,'Closed','Under Review','Resolved'),date(c.created_at)])):empty('لا شكاوى','قدّم شكوى من النموذج أعلاه.');
    const maintenanceTable=maintenance.items.length?dataTable(['العطل','الوصف','الحالة','أُنشئ في'],maintenance.items.map(m=>[m.problem_type,m.description,d5badge(m.status,'Closed','Assigned','Resolved'),date(m.created_at)])):empty('لا طلبات صيانة','قدّم طلب صيانة من النموذج أعلاه.');
    renderPage('support',epoch,heading('الدعم والبلاغات','الشكاوى تصل لإدارة السكن وطلبات الصيانة لمسؤول الصيانة.'),panel('شكوى جديدة',complaintForm),panel('شكواي',complaintTable),panel('طلب صيانة جديد',maintenanceForm),panel('طلباتي',maintenanceTable));
  }
  async function complaintsAdmin(epoch){const list=await api('/complaints');
    function act(row,action,label,kind='secondary'){return button(label,async()=>{const resolution=action==='start'?null:window.prompt('نص المعالجة (إلزامي للإنهاء):');if(action!=='start'&&!resolution)return;try{await api('/complaints/'+row.complaint_id+'/action',{method:'POST',body:{action,resolution}});notify('حُدّثت الشكوى.');await complaintsAdmin(epoch);}catch(err){notify(err.message,true);}},'kind'===kind);}
    function rowActions(c){const acts=[];if(c.status==='Open')acts.push(act(c,'start','بدء المراجعة','primary'));if(c.status==='Under Review')acts.push(act(c,'resolve','حلّ الشكوى','primary'));if(c.status==='Resolved')acts.push(act(c,'close','إغلاق'));return acts.length?h('span',{class:'file-actions'},acts):'';}
    const table=list.items.length?dataTable(['الطالب','التصنيف','الوصف','الحالة',''],list.items.map(c=>[c.student_name||'—',c.category,c.description,d5badge(c.status,'Closed','Under Review','Resolved'),rowActions(c)])):empty('لا شكاوى','الشكاوى المقدمة من الطلاب تظهر هنا.');
    renderPage('support',epoch,heading('شكاوى الطلاب','من الاستلام إلى الإغلاق، مع نص المعالجة.'),panel('الشكاوى',table));
  }
  async function maintenanceAdmin(epoch){const list=await api('/maintenance');
    function act(row,action,label,kind='secondary'){return button(label,async()=>{const resolution=['resolve','close'].includes(action)?window.prompt('نص المعالجة (إلزامي):'):null;if(['resolve','close'].includes(action)&&!resolution)return;try{await api('/maintenance/'+row.request_id+'/action',{method:'POST',body:{action,resolution}});notify('حُدّث الطلب.');await maintenanceAdmin(epoch);}catch(err){notify(err.message,true);}},'kind'===kind);}
    function rowActions(m){const acts=[];if(m.status==='Pending')acts.push(act(m,'assign','إسناد','primary'));if(m.status==='Assigned')acts.push(act(m,'progress','بدء التنفيذ','primary'));if(m.status==='In Progress')acts.push(act(m,'resolve','تمت المعالجة','primary'));if(m.status==='Resolved')acts.push(act(m,'close','إغلاق'));return acts.length?h('span',{class:'file-actions'},acts):'';}
    const table=list.items.length?dataTable(['الطالب','العطل','الوصف','الغرفة','الحالة',''],list.items.map(m=>[m.student_name||'—',m.problem_type,m.description,m.room_number?m.room_number+' · '+(m.floor_number?'ط'+m.floor_number:'')+' · '+(m.building_name||''):'—',d5badge(m.status,'Closed','Assigned','Resolved'),rowActions(m)])):empty('لا طلبات صيانة','طلبات الصيانة من الطلاب تظهر هنا.');
    renderPage('support',epoch,heading('طلبات الصيانة','إسناد، تنفيذ، وإغلاق مع التوثيق.'),panel('الطلبات',table));
  }
  async function attendancePage(){const epoch=viewEpoch;if(state.page!=='attendance'||!state.me)return;
    if(state.me.role==='Student')return studentAttendance(epoch);
    if(state.me.role==='Student Affairs')return attendanceAdmin(epoch);
    renderPage('attendance',epoch,heading('الموافقات والغياب','لا صلاحيات لهذا الدور.'),h('p',{class:'muted'},'هذه الصفحة للطلاب وشؤون الطلاب.'));}
  async function studentAttendance(epoch){const permissions=await api('/permissions/my');const reports=await api('/emergency/my');const absences=await api('/absences/my');const ledger=await api('/attendance/my-ledger?limit=50');
    const permissionForm=h('form',{class:'inline-form'},field('من','start_date','','date'),field('إلى','end_date','','date'),field('السبب','reason','','text',{minlength:3,maxlength:2000}),h('button',{type:'submit',class:'primary'},'طلب إذن غياب'));
    permissionForm.addEventListener('submit',async e=>{e.preventDefault();const b=permissionForm.querySelector('button');b.disabled=true;try{await api('/permissions',{method:'POST',body:Object.fromEntries(new FormData(permissionForm))});notify('أُرسل الطلب لشؤون الطلاب.');await studentAttendance(epoch);}catch(err){notify(err.message,true);}finally{b.disabled=false;}});
    const reportForm=h('form',{class:'inline-form'},field('وصف الحالة','description','','text',{minlength:3,maxlength:2000}),h('button',{type:'submit',class:'primary'},'بلاغ حالة طارئة'));
    reportForm.addEventListener('submit',async e=>{e.preventDefault();const b=reportForm.querySelector('button');b.disabled=true;try{await api('/emergency',{method:'POST',body:Object.fromEntries(new FormData(reportForm))});notify('وصل البلاغ.');await studentAttendance(epoch);}catch(err){notify(err.message,true);}finally{b.disabled=false;}});
    const permissionTable=permissions.items.length?dataTable(['الفترة','السبب','الحالة',''],permissions.items.map(p=>[p.start_date+' → '+p.end_date,p.reason,d5badge(p.status,'Approved','Pending','Rejected'),p.status==='Pending'?button('إلغاء الطلب',async()=>{try{await api('/permissions/'+p.permission_id+'/cancel',{method:'POST'});notify('أُلغي الطلب.');await studentAttendance(epoch);}catch(err){notify(err.message,true);}},'danger'):''])):empty('لا طلبات إذن','الطلبات المقررة من شؤون الطلاب تُسجل في سجل الغياب.');
    const reportTable=reports.items.length?dataTable(['الوصف','الحالة','الخروج','أُبلغ في'],reports.items.map(r=>[r.description,d5badge(r.status,'Verified','Reported','Closed'),r.exit_verified?'موثّق':'بانتظار التحقق',date(r.reported_at)])):empty('لا بلاغات طارئة','بلاغات الخروج الطارئ تُوثق هنا بعد التحقق.');
    const absenceTable=absences.items.length?dataTable(['النوع','الفترة','المصدر'],absences.items.map(a=>[tr(a.absence_type),a.start_date+(a.end_date?' → '+a.end_date:''),a.source||'—'])):empty('لا غياب مسجل','كل غياب موثق من الإذونات أو البلاغات.');
    renderPage('attendance',epoch,heading('الغياب والإذن','طلبات إذن الغياب، بلاغات الخروج الطارئ، وسجل الغياب الموثق.'),panel('طلب إذن غياب',permissionForm),panel('بلاغ حالة طارئة',reportForm),panel('طلبات الإذن',permissionTable),panel('بلاغاتي',reportTable),panel('سجل غيابي',absenceTable),
    panel('سجل الحضور اليومي',ledger.items.length?dataTable(['التاريخ','الحالة','ملاحظات','المصدر'],ledger.items.map(l=>[l.record_date,d5badge(l.status,'Present','Late','Absent'),l.notes||'—',l.source==='bulk'?'دفعة':'موظف'])):empty('لا سجلات حضور بعد','يسجل مسؤول الإسكان حضورك اليومي؛ الغياب دون إذن أو بلاغ موثق يُسجل في سجل الغياب.')));
  }
  async function attendanceAdmin(epoch){const permissions=await api('/permissions');const reports=await api('/emergency');const absences=await api('/absences');
    function decide(p,action,label,kind='primary'){return button(label,async()=>{try{await api('/permissions/'+p.permission_id+'/decision',{method:'POST',body:{action}});notify('حُدّث الطلب.');await attendanceAdmin(epoch);}catch(err){notify(err.message,true);}},'kind'===kind);}
    const permissionTable=permissions.items.length?dataTable(['الطالب','الفترة','السبب','الحالة',''],permissions.items.map(p=>[p.student_name||'—',p.start_date+' → '+p.end_date,p.reason,d5badge(p.status,'Approved','Pending','Rejected'),p.status==='Pending'?h('span',{class:'file-actions'},decide(p,'approve','موافقة','primary'),decide(p,'reject','رفض','danger')):''])):empty('لا طلبات إذن','طلبات الطلاب تظهر هنا للموافقة أو الرفض.');
    function reportAction(r,action,label,kind='primary'){return button(label,async()=>{try{await api('/emergency/'+r.report_id+'/action',{method:'POST',body:{action}});notify('حُدّث البلاغ.');await attendanceAdmin(epoch);}catch(err){notify(err.message,true);}},'kind'===kind);}
  ...
  async function dailyAttendancePage(){const epoch=viewEpoch;if(state.page!=='daily'||!state.me)return;
    const today=new Date().toISOString().slice(0,10);
    const pending=[];
    const dateInput=h('input',{type:'date',value:today,'aria-label':'تاريخ اليوم',style:'max-width:14rem'});
    const searchInput=h('input',{type:'search',placeholder:'ابحث بالاسم…','aria-label':'بحث عن طالب',maxlength:100,style:'max-width:12rem'});
    const studentSelect=h('select',{'aria-label':'الطالب'});
    const statusSelect=h('select',{'aria-label':'الحالة'},Object.entries(attendanceStatuses).map(([v,l])=>h('option',{value:v},l)));
    const notesInput=h('input',{type:'text',maxlength:500,'aria-label':'ملاحظات',placeholder:'ملاحظات اختيارية'});
    const pendingBox=h('div',{});
    function renderPending(){pendingBox.replaceChildren(pending.length?pending.map((e,i)=>h('div',{class:'document-row'},h('div',{},h('strong',{},e.student_name),h('small',{},tr(e.status)+(e.notes?' · '+e.notes:'')),h('small',{},e.student_id)),button('إزالة',async()=>{pending.splice(i,1);renderPending();},'danger'))):h('p',{class:'muted'},'لم تُضف سجلات بعد.'));}
    async function loadStudents(filter=''){const list=await api('/attendance/students?limit=100'+(filter?'&q='+encodeURIComponent(filter):''));studentSelect.replaceChildren(h('option',{value:''},'اختر الطالب'+(list.total>100?' (يعرض 100 من '+list.total+')':'')),...list.items.map(st=>h('option',{value:st.student_id},st.full_name+' · '+(st.major||st.university||''))));studentSelect.value='';}
    searchInput.addEventListener('input',()=>{clearTimeout(searchInput._t);searchInput._t=setTimeout(()=>loadStudents(searchInput.value.trim()).catch(e=>notify(e.message,true)),300);});
    async function loadDay(){const day=dateInput.value||today;const page=await api('/attendance/daily?record_date='+day+'&limit=100');
      const rows=page.items.length?dataTable(['الطالب','الحالة','ملاحظات','المصدر'],page.items.map(r=>[r.student_name||r.student_id,d5badge(r.status,'Present','Late','Absent'),r.notes||'—',r.source==='bulk'?'دفعة':'موظف'])):empty('لا سجلات لهذا اليوم','سجّل الحضور من النموذج أعلاه.');
      renderPage('daily',epoch,heading('الحضور اليومي','تسجيل حضور الطلاب لكل يوم؛ الغياب دون إذن موثق يُسجل غياباً غير مصرح تلقائياً.'),
        panel('تسجيل الحضور',h('div',{class:'inline-form'},dateInput,searchInput),h('div',{class:'inline-form'},studentSelect,statusSelect,notesInput,button('إضافة إلى السجل',async()=>{const sid=studentSelect.value;if(!sid){notify('اختر الطالب.',true);return;}if(pending.some(e=>e.student_id===sid)){notify('الطالب مضاف مسبقاً.',true);return;}const all=await api('/attendance/students?limit=100');const chosen=all.items.find(x=>x.student_id===sid);pending.push({student_id:sid,student_name:chosen?chosen.full_name:sid,status:statusSelect.value,notes:notesInput.value.trim()||null});notesInput.value='';studentSelect.value='';renderPending();})),
        pendingBox,h('div',{class:'actions'},button('حفظ سجل اليوم',async()=>{if(!pending.length){notify('أضف سجلاً واحداً على الأقل.',true);return;}const day=dateInput.value||today;try{await api('/attendance/daily',{method:'POST',body:{record_date:day,records:pending.map(({student_id,status,notes})=>({student_id,status,notes}))}});notify('حُفظ سجل اليوم.');pending.length=0;renderPending();await loadDay();}catch(err){notify(err.message,true);}},'primary'))),
        panel('سجل اليوم — '+day,rows,h('p',{class:'hint'},'سجل واحد لكل طالب في اليوم؛ إعادة التسجيل تصحح الحالة، ويبقى أثر الغياب الموثق محفوظاً.')));
    }
